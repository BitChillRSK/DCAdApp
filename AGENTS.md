...
8. **A schedule is keyed by `(token, scheduleId)`, and the stored owner is checked in exactly one place** — `s_dcaSchedules` is `mapping(address token => mapping(uint64 scheduleId => DcaSchedule))`. The stablecoin is keyed first because that is how the work arrives: a batch is one handler's, so one stablecoin's, and its rows are ids. Reach a schedule through `_callersSchedule(token, scheduleId)`, which is the only ownership check in the contract: it refuses a pair that addresses nothing (`InexistentSchedule`) and then refuses a schedule whose stored `user` is not `msg.sender` (`NotScheduleOwner`). Do not add a second owner check anywhere, and never accept a `user` argument on a schedule mutator — a caller-supplied owner is not a check. A reviewer's obligation is one grep: no mutator may read `s_dcaSchedules` directly. The `onlySwapper` purchase path needs no ownership check at all, because it takes the buyer from the schedule rather than from the batch; the stablecoin it names is structural, so a row of another one addresses empty storage and is refused rather than debited by a handler that never held its funds. Every getter is free to read any schedule. `DcaSchedule` is two slots and holds neither half of its key: slot 0 is every field a purchase touches, slot 1 pairs the owner with a `uint96 purchaseAmount`. There is no second, storage-only schedule type — `getDcaSchedules` returns the ids in a parallel array instead of repeating them inside the value. `s_scheduleIds[user][token]` is enumeration only: `getDcaSchedules` and the max-schedules bound read it, and no purchase ever does. See `docs/relaunch/R64-batch-calldata-and-schedule-keying.md`.

9. **A batch row is one `uint64` and nothing else** — `Batch` names schedule ids, plus the `token` and `routeIndex` that resolve to one handler and the batch's `minRbtcOut`. `_batchBuyRbtc` reads both the buyer and the amount from each schedule. Do not add a per-row buyer back: the owner comes from storage, which is what lets a row be an id rather than an id plus an address, and a caller-supplied owner would be trusted input on the one path that reaches somebody else's schedule. Do not add a per-row amount back as a staleness guard either: it was measured at ~450 gas per row, and what it bought was turning one user's mid-flight edit into a failed tick for every other row in the batch, in order to refuse a purchase at the amount that user had just asked for. `minRbtcOut` still binds on the rBTC the handler measures itself receiving, and a Uniswap route's oracle floor still scales with the actual input, so nothing about slippage rests on the caller's arithmetic. The `routeIndex` comparison is not the same thing and stays: no setter can change a schedule's route, so it is an integrity check that the row belongs to this batch's handler, not a guard against staleness.

10. **The protected purchase window is opt-in, exact, and bounded** — an address currently on the swapper allowlist may activate a five-block window whenever none is live. Activation in block `N` blocks exactly `updatePurchaseAmount`, `updatePurchasePeriod`, `setSchedulePaused`, `deleteDcaSchedule`, `withdrawToken`, `withdrawTokenAndInterest`, and `withdrawAllAccumulatedInterest` through `N + 4`; those calls resume at `N + 5`. An active window cannot be extended or renewed early; once it expires the swapper may activate again with no daily budget. Deposits, creation, interest top-ups, accumulated-rBTC withdrawals, purchases, reads, and governance setters stay open, and purchases never require activation. The bot waits for the activation receipt, refreshes or simulates from the locked state, and only then broadcasts the purchase within the window; five blocks are an execution buffer, not a finality period. Do not make the duration configurable, allow mid-window renewal, add a lock branch to purchases, or broaden the guarded set without a new product decision. See `docs/relaunch/R66-batch-row-front-running.md`.

11. **Exact external share consumption on every successful lending redeem** — after a successful lending redemption (principal, interest, or batch purchase), the handler's external receipt-share balance must decrease by exactly the amount debited from BitChill's per-user virtual books. Measure that decrease with `balanceOf(handler)` for Sovryn iTokens / Tropykus kTokens and `scaledBalanceOf(handler)` for LayerBank aTokens; do not trust a protocol return value. A smaller stablecoin payout is allowed only when that share claim was fully consumed (exit fee / realized loss). Positive cash with a partial share burn must revert the whole operation so no still-withdrawable claim is orphaned. Protocols that silently partial-fill or use async request/claim queues need a separate handler lifecycle, not a tolerance on this check. See `docs/relaunch/R68-lending-redeem-exact-consumption.md`.
...
- `make slither` / `make aderyn` if installed; not part of `make check` (findings are triaged at cutover in R73, not a clean zero baseline).
...
3. **Push and open a PR** only after `make check`, `make fork-sovryn`, and `make fork-tropykus` pass. Use `.github/PULL_REQUEST_TEMPLATE.md`. Point at the spec. Do not commit secrets or `.env`.
