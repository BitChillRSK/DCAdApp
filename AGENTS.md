...
10. **The protected purchase window is opt-in, exact, and bounded** — an address currently on the swapper allowlist may activate one five-block window per UTC day. Activation in block `N` blocks exactly `updatePurchaseAmount`, `updatePurchasePeriod`, `setSchedulePaused`, `deleteDcaSchedule`, `withdrawToken`, `withdrawTokenAndInterest`, and `withdrawAllAccumulatedInterest` through `N + 4`; those calls resume at `N + 5`. A next-day activation cannot extend a window that is still active. Deposits, creation, interest top-ups, accumulated-rBTC withdrawals, purchases, reads, and governance setters stay open, and purchases never require activation. The bot waits for the activation receipt, refreshes or simulates from the locked state, and only then broadcasts the purchase within the window; five blocks are an execution buffer, not a finality period. Do not make the duration configurable or renewable, add a lock branch to purchases, or broaden the guarded set without a new product decision. See `docs/relaunch/R66-batch-row-front-running.md`.
